...
from langchain_community.vectorstores import Chroma  # Actualización aquí
from langchain_community.embeddings import HuggingFaceEmbeddings  # Actualización aquí
from langchain_ollama import OllamaLLM # Actualización aquí
...
def buscar_informacion_langchain(df, query):
    # Crear un vectorstore básico con los datos del DataFrame
    embeddings = HuggingFaceEmbeddings()
    
    # Convertir las filas del DataFrame en documentos
    documents = [Document(page_content=row.to_json()) for _, row in df.iterrows()]
    ...
    qa_chain = RetrievalQA.from_chain_type(llm=OllamaLLM(), retriever=retriever)
